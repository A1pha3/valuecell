...
> **本节假设你已阅读 [architecture.md](./architecture.md) 的 §4 和 §6 节，对系统整体结构有基本认知。**
...
### 2.3 可选实现：`notify()` 方法（推送通知）

`BaseAgent` 还提供了可选的 `notify()` 方法，用于 Agent **主动推送**通知。与 `stream()` 的关键区别：

| 维度 | `stream()` | `notify()` |
|------|-----------|-----------|
| 装饰器 | `@abstractmethod`（**必须**实现） | 无装饰器（**可选**覆盖） |
| 触发方式 | 用户主动发起请求 | Agent 主动推送（如定时任务、文件监控） |
| 返回类型 | `AsyncGenerator[StreamResponse, None]` | `AsyncGenerator[NotifyResponse, None]` |
| 事件命名空间 | `streaming.*` | `notification.*` |

如果 Agent 不需要主动推送功能，可以完全忽略 `notify()` 方法——基类提供了一个空的默认实现（`raise NotImplementedError`），但因为没有 `@abstractmethod` 装饰器，Python 不会强制子类覆盖它。只有需要主动推送通知（如定时新闻推送、文件监控告警）的 Agent 才需要覆盖此方法。

**实现示例**：

```python
from valuecell.core.agent import notification


class NewsAgent(BaseAgent):
    # ... stream() 实现 ...

    async def notify(self, query, conversation_id, task_id, dependencies=None):
        """定时新闻推送或文件监控通知。"""
        yield notification.message(f"新闻更新：{query}")
        yield notification.done()
```

> **注意**：`notification.component_generator()` 的返回类型是 `StreamResponse`（不是 `NotifyResponse`），这是设计上的一个特殊处理——组件事件在两种管道中共享同一种响应模型。详见 [streaming-deep-dive.md](./streaming-deep-dive.md) §9 节。

### 2.4 创建独立运行入口

在 `__main__.py` 中，使用 `create_wrapped_agent()` 包装并启动。这是将你的 Agent 注册为 A2A 服务的关键步骤：
...
1. **查找 Agent Card**：调用 `find_local_agent_card_by_agent_name(agent_class.__name__)`，在 `python/configs/agent_cards/*.json` 中按 `name` 字段匹配。如果找不到匹配的 Card，抛出 `ValueError`。
2. **包装为 A2A 服务**：通过 `_serve(agent_card)` 装饰器将你的类包装为 `DecoratedAgent`，自动注入 `serve()` 和 `shutdown()` 方法。
3. **提取端口配置**：从 Card 的 `url` 字段解析 host 和 port（`parse_host_port(agent_card.url)`），配置 uvicorn 监听地址。
4. **返回实例**：注意 `create_wrapped_agent(HelloAgent)` 返回的是**已实例化的对象**，不是类。返回值可直接调用 `.serve()` 启动服务。
...
> **注意**：当前 `python/configs/agents/` 目录下只有 `super_agent.yaml`、`research_agent.yaml`、`news_agent.yaml`。`PromptBasedStrategyAgent` 和 `GridStrategyAgent` 没有 Agent YAML 配置文件，它们使用默认模型配置。

新增 Agent 时，请选择未被占用的端口。建议使用 10010 以上的端口，避免与系统 Agent（10004-10007）冲突。
...
| `COMPONENT_GENERATOR` | `streaming.component_generator(component_type, content)` | 富 UI 组件 |
| `TASK_COMPLETED` | `streaming.done()` | 任务成功完成（**不是** `SystemResponseEvent.DONE`） |
| `TASK_FAILED` | `streaming.failed(content)` | 任务失败 |

> **重要区分**：`streaming.done()` 创建的是 `TaskStatusEvent.TASK_COMPLETED`（任务级完成），而 `SystemResponseEvent.DONE`（会话级结束）由编排器在所有任务完成后自动发出。Agent 只需调用 `streaming.done()` 标记自己的任务完成即可。
...
工具内部的异常应被捕获并转换为友好的返回值，而不是让异常传播到 Agent 层导致整个任务失败。以下是一个推荐模式：
...
def search_stock_info(ticker: str) -> str:
    ...
    try:
        ...
        # 工具内部的异常不应抛到 Agent 层
        return f"查询 {ticker} 时出错：{str(e)}"
```

这种模式确保了：
- Agent 不会因单个工具失败而崩溃
- 用户能看到有意义的错误信息
- LLM 可以根据错误信息决定是否重试或换一种方式
...
从零开始构建一个功能完整的 Agent 并非一蹴而就。以下是一个分阶段递进的路线图，帮助你从最简实现逐步演进到生产级 Agent。

### Level 1：Echo Agent（最小流式响应）

**目标**：实现一个只回显用户输入的 Agent，验证三文件闭环。

**核心概念**：

- 继承 `BaseAgent` 并实现 `stream()` 方法
- 使用 `streaming.message_chunk()` 发送文本片段
- 以 `streaming.done()` 结束响应流

**参考实现**：本文 §2.2 的 `HelloAgent` 就是一个标准的 Echo Agent。

**通过标志**：能在前端看到流式返回的文本，且前端正确识别响应结束。

### Level 2：LLM-powered Agent（接入大语言模型）

**目标**：在 Agent 中集成 LLM，使 Agent 具备自然语言推理能力。

**新增概念**：

- 使用 `get_model_for_agent()` 加载模型配置
- 创建 Agno `Agent` 实例并注册工具函数
- 将 Agno 事件（`RunContent`、`ToolCallStarted` 等）映射为 ValueCell 的 `streaming.*` 事件
- 工具函数的文档字符串编写与错误处理

**参考实现**：本文 §5.2 的 `ResearchAgent` 示例。

**通过标志**：Agent 能根据用户意图调用 LLM，并在前端展示推理过程和工具调用。

### Level 3：Knowledge-based Agent（知识库增强）

**目标**：为 Agent 接入知识库，使其能检索特定领域的数据。

**新增概念**：

- Agno 的知识库（Knowledge Base）集成
- 向量检索与上下文注入
- `COMPONENT_GENERATOR` 事件的使用（如 `report`、`filtered_line_chart` 等富 UI 组件）

**通过标志**：Agent 能基于知识库内容回答领域问题，并以富 UI 组件呈现结构化数据。

### Level 4：Notification Agent（主动推送）

**目标**：让 Agent 支持主动推送通知，如定时任务触发、文件变更告警。

**新增概念**：

- 覆盖 `notify()` 方法实现推送逻辑
- 使用 `notification.*` 辅助函数
- 定时任务与事件驱动的触发机制
- `notification.component_generator()` 与 `streaming.component_generator()` 的类型差异

**参考实现**：本文 §2.3 的 `NewsAgent` 示例。

**通过标志**：Agent 能在无人请求的情况下主动向前端推送消息，且推送内容在前端正确渲染。

> **渐进式开发建议**：不要试图一步到达 Level 4。每完成一个 Level，确保闭环跑通、前端验证通过后，再进入下一阶段。Level 1 的闭环是所有后续工作的基础。

---

## 常见问题

### Q：Agent 启动后无法被系统发现，怎么办？

这是最常见的接入问题，通常由以下原因导致：

1. **Card 中的 `name` 字段与类名不一致**：`create_wrapped_agent()` 会用 `agent_class.__name__` 去 `python/configs/agent_cards/*.json` 中匹配 `name` 字段。如果 Card 里写的是 `"name": "hello_agent"` 而类名是 `HelloAgent`，匹配就会失败。请确保二者完全一致，包括大小写。

2. **缺少 Agent Card 文件**：确认 `python/configs/agent_cards/` 目录下存在对应的 `.json` 文件，且文件内容是合法的 JSON。

3. **缺少 `__main__.py` 文件**：后端服务器通过 `uv run -m valuecell.agents.<agent_name>` 启动 Agent，`__main__.py` 是必需的入口文件。

4. **端口冲突**：Card 中 `url` 字段指定的端口如果已被其他 Agent 占用，会导致启动失败。参考 §3.3 的端口分配表选择未被占用的端口。

### Q：`stream()` 方法中可以发 HTTP 请求吗？

可以。`stream()` 是 `async` 方法，你可以使用 `httpx`、`aiohttp` 等异步 HTTP 客户端在方法内部发起网络请求。不过，如果这个 HTTP 请求的目的是获取结构化数据供 LLM 使用，建议将其封装为一个工具函数（Tool），通过 Agno 的工具机制调用。这样做有三个好处：

- LLM 可以自主决定是否调用，而不是每次都执行
- 调用过程会通过 `ToolCallStarted` / `ToolCallCompleted` 事件展示给用户
- 工具函数可以被其他 Agent 复用

### Q：如何让 Agent 支持多种语言？

`stream()` 方法的 `dependencies` 参数可以传递上下文信息，包括用户的语言偏好。你可以与前端约定一个键名（如 `"language"`），在 `stream()` 内部根据该值选择不同的提示词或响应模板：

```python
async def stream(self, query, conversation_id, task_id, dependencies=None):
    lang = (dependencies or {}).get("language", "zh")
    if lang == "en":
        yield streaming.message_chunk("Processing your request...")
    else:
        yield streaming.message_chunk("正在处理您的请求...")
    # ...
    yield streaming.done()
```

注意，`dependencies` 的具体内容由调用方（编排器或前端）决定，你需要与前端团队协商字段约定。

### Q：`notify()` 方法和 `stream()` 方法可以共享状态吗？

可以。`notify()` 和 `stream()` 是同一个 Agent 实例上的方法，因此可以通过 `self` 共享实例属性。例如：
...
class MyAgent(BaseAgent):
    ...
    async def stream(self, query, conversation_id, task_id, dependencies=None):
        if self._latest_data:
            yield streaming.message_chunk(f"最近的数据：{self._latest_data}")
        # ...
        yield streaming.done()

    async def notify(self, query, conversation_id, task_id, dependencies=None):
        # 后台定时更新数据
        self._latest_data = "新获取的数据"
        yield notification.message("数据已更新")
        yield notification.done()
```

**线程安全提示**：如果 `notify()` 由定时任务在独立线程中触发，而 `stream()` 在 asyncio 事件循环中运行，需要确保对共享状态的读写是线程安全的。可以使用 `asyncio.Lock` 或 `threading.Lock` 保护临界区。

### Q：如何调试 Agent 的工具调用？

推荐两种方式：

1. **启用调试模式**：在 `.env` 文件中设置 `AGENT_DEBUG_MODE=true`（详见本文 §6）。调试模式会记录发送给模型的完整 Prompt、工具调用详情和中间步骤。

2. **控制台日志**：在工具函数内部使用 Python 标准的 `logging` 模块输出调试信息：
...
def search_stock_info(ticker: str) -> str:
    """查询股票信息。"""
    logger.info("Tool called: search_stock_info(ticker=%s)", ticker)
    try:
        result = f"Stock info for {ticker}"
...
可以。Agno 只是 ValueCell 推荐的 Agent 构建框架，不是强制依赖。你的 `stream()` 方法唯一需要满足的契约是：以 `yield` 逐条返回 ValueCell 的 `StreamResponse` 事件，并以 `streaming.done()` 结束。你可以用任何方式生成这些事件：

- 直接用 `httpx` 调用 LLM 的 HTTP API，手动解析 SSE 流并转换为 ValueCell 事件
- 使用 LangChain、LlamaIndex 等其他框架
- 甚至不使用 LLM，用纯规则逻辑生成响应

Agno 的优势在于它已经封装了模型调用、工具注册、知识检索等常用功能，能减少样板代码。但如果你有特定需求，完全可以绕过它。

---

## 自测检查清单

完成本文学习后，用以下清单验证你的掌握程度。每个问题都应该能不查阅文档直接回答。

1. **三文件清单**：能否列出一个可工作的 Agent 所需的全部三个文件，并说明各自的位置和职责？（提示：Python 实现、Agent Card、Agent YAML。详见 §1。）

2. **`stream()` 为什么必须是 async**：能否解释为什么 `stream()` 必须是 `async def` 而不是普通同步函数？（提示：流式架构、网络调用、yield 逐条产出。详见 §2.2。）

3. **`streaming.done()` 与 `SystemResponseEvent.DONE` 的区别**：能否说明二者的发出者不同、层级不同？（提示：`streaming.done()` 是任务级完成，由 Agent 发出；`SystemResponseEvent.DONE` 是会话级结束，由编排器自动发出。详见 §4.2。）

4. **`create_wrapped_agent()` 的职责**：能否说出它做了哪四件事？（提示：查找 Agent Card、包装为 A2A 服务、提取端口配置、返回实例。详见 §2.4。）

5. **`notify()` 与 `stream()` 的使用场景**：能否说明何时该用 `notify()` 而非 `stream()`？（提示：`stream()` 是被动响应用户请求，`notify()` 是 Agent 主动推送。详见 §2.3。）

6. **Agent Card 的 `name` 字段约束**：能否说明该字段必须与什么保持一致，不一致会导致什么后果？（提示：必须与 Python 类名完全一致，否则 `create_wrapped_agent()` 匹配失败。详见 §3.1。）

7. **工具函数的文档字符串**：能否解释为什么 Agno 的工具函数必须有清晰的文档字符串？（提示：Agno 会将 docstring 作为工具描述传给 LLM，它决定了 LLM 能否正确选择和调用工具。详见 §5.2。）

8. **调试模式的风险**：能否说明为什么调试模式不应在生产环境开启？（提示：会记录敏感的输入输出内容，并显著增加日志量和延迟。详见 §6。）

如果以上 8 个问题你都能自信回答，说明你已经具备了独立开发 ValueCell Agent 的基础能力。
